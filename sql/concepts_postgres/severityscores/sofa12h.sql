...
DROP TABLE IF EXISTS sofa12h; CREATE TABLE sofa12h AS 
-- ------------------------------------------------------------------
...
(
  ...
  inner join inputevents_cv cv
    on ie.icustay_id = cv.icustay_id and cv.charttime between ie.intime and DATETIME_ADD(ie.intime, INTERVAL '12' HOUR)
  left join wt
...
(
  ...
  inner join inputevents_mv mv
    on ie.icustay_id = mv.icustay_id and mv.starttime between ie.intime and DATETIME_ADD(ie.intime, INTERVAL '12' HOUR)
...
(
  ...
  from blood_gas_12h_arterial bg
...
left join vitals_12h v
  on ie.icustay_id = v.icustay_id
left join labs_12h l
  on ie.icustay_id = l.icustay_id
left join urine_output_12h uo
  on ie.icustay_id = uo.icustay_id
left join gcs_12h gcs
...
(
  ...
  , case
    ...
    when  (urineoutput * 2) < 200 then 4
    when (creatinine_max >= 3.5 and creatinine_max < 5.0) then 3
    when  (urineoutput *2) < 500 then 3
