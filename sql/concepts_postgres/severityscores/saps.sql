-- 1. Suchpfad setzen: Erst bei den abgeleiteten Tabellen schauen, dann in public
SET search_path TO mimiciii_derived, public;

-- 2. Tabelle im derived-Schema erstellen
DROP TABLE IF EXISTS mimiciii_derived.sapsii;
CREATE TABLE mimiciii_derived.sapsii AS
...
(
  ...
  -- WICHTIG: Hier steht jetzt nur noch "icustays", Postgres findet es via search_path in "public"
  FROM icustays ie
...
select hadm_id
  ...
  , max(CASE
    ...
    when SUBSTR(icd9_code,1,4) = '2386' then 1
    when SUBSTR(icd9_code,1,4) = '2733' then 1
        end) as hem
  , max(CASE
...
(
  ...
  -- Hier greift der search_path auf mimiciii_derived zu:
...
-- join to custom tables (gefunden via search_path in mimiciii_derived)
